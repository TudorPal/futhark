...
async function run() {
  ...
  log(hasWasm ? "WebAssembly available." : "WebAssembly NOT available.");
  ...
  if (!hasWasm) {
    log("");
    log("Cannot run: WebAssembly is missing.");
    return;
  }
  ...
  async function frame(now) {
    ...
    if (now - lastStatsNow >= 1000) {
      ...
      log(
        `FPS: ${fps.toFixed(1)} | avg frame compute: ${avgCompute.toFixed(2)} ms | speed: ${currentSpeed().toFixed(1)}x`
      );
